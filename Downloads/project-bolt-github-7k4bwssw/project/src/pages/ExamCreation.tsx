...
export const ExamCreation: React.FC = () => {
  ...
  const [form, setForm] = useState<ExamForm>({
    ...
    module: '',
    sponsorDS: '',
    instructions: 'Please fill the bubbles completely with a dark pencil. Mark only one answer per question.',
  });
  ...
  const validateExamForm = () => {
    ...
    if (!form.dateTime) newErrors.push('Date and time is required');
  ...
  const handleCreateExam = async (e: React.FormEvent) => {
    ...
    try {
      const response = await api.post('/exams', form);
      setExamId(response.data.examId);
      setCurrentStep(2);
    } catch (error: any) {
      setErrors([error.response?.data?.error || 'Failed to create exam']);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleStudentUpload = async (file: File) => {
    ...
    try {
      ...
      setErrors([error.response?.data?.error || 'Failed to upload students']);
    } finally {
      setLoading(false);
    }
  };
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm(prev => ({ ...prev, numQuestions: parseInt(e.target.value) }))}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm(prev => ({ ...prev, marksPerMcq: parseFloat(e.target.value) }))}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setForm(prev => ({ ...prev, passingPercentage: parseInt(e.target.value) }))}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  required
                />
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
              <div>
                ...
                <div className="space-y-2 text-sm">
                  ...
                  <div><span className="text-gray-600">Date:</span> <span className="ml-2">{new Date(form.dateTime).toLocaleString()}</span></div>
