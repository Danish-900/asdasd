...
export const ScanProcess: React.FC = () => {
  ...
  const downloadResultsPDF = async () => {
    ...
    try {
      ...
      // Prepare results data for scanned sheets
      ...
      console.log("Downloading scanned sheets with data:", JSON.stringify(resultsData, null, 2));
      console.log("Files:", selectedFiles.map(file => file.name));
      console.log("Request URL:", `${api.defaults.baseURL || 'http://localhost:3001'}/api/scan/download-scanned-sheets`);
      ...
      const response = await api.post('/scan/download-scanned-sheets', formData, {
      ...
      link.href = url;
      link.download = `${selectedExamData?.name || 'Exam'}_Scanned_Sheets_${new Date().toISOString().split('T')[0]}.pdf`;
      link.click();
      window.URL.revokeObjectURL(url);
      
      setSuccess('Scanned sheets downloaded successfully!');
    } catch (error: any) {
      console.error('Scanned sheets download failed:', {
        url: `${api.defaults.baseURL || 'http://localhost:3001'}/api/scan/download-scanned-sheets`,
        error: error.response?.data || error.message,
        status: error.response?.status,
      });
      setError(`Failed to download scanned sheets: ${error.response?.data?.detail || error.message}`);
    }
  };
              ...
              <div className="flex space-x-3">
                ...
                {results.length > 0 && !isScanning && (
                  ...
                  >
                    ...
                    <span>Download Scanned Sheets</span>
                  </button>
                )}
              </div>
            </div>
              ...
              <div className="space-y-4">
                {filteredResults.map((result, index) => (
                  ...
                  >
                    ...
                    {/* Display extracted student credentials */}
                    <div className="mt-3 pt-3 border-t border-gray-200">
                      <p className="text-sm font-medium text-gray-700 mb-2">Extracted Student Information:</p>
