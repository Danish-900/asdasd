...
def extract_student_credentials_ocr(image_data: bytes) -> Dict[str, str]:
    """Extract student credentials from OMR sheet using OCR."""
    try:
        ...
        # Define the region where student information is typically located
        # Assuming student info box is in the upper portion of the sheet
        width, height = image.size
        
        # Crop to student information area (adjust coordinates as needed)
        # Typically in the top 40% of the image, left side
        student_info_box = image.crop((
            int(width * 0.05),  # left
            int(height * 0.25), # top
            int(width * 0.5),   # right
        ...
        student_info_box = student_info_box.resize(
            (student_info_box.width * 2, student_info_box.height * 2), 
            Image.Resampling.LANCZOS
        )
        
        # Convert to grayscale for better OCR
        gray_image = student_info_box.convert('L')
        
        # Apply OCR
        ocr_text = pytesseract.image_to_string(gray_image, config='--psm 6')
        
        logger.info(f"OCR extracted text: {ocr_text}")
        
        # Parse the extracted text to find student credentials
        credentials = {
            'name': 'Unknown',
            'rank': 'Unknown',
        ...
        for line in lines:
            ...
            if 'name' in line_lower and ':' in line:
                ...
                if name_match:
                    credentials['name'] = name_match.group(1).strip()
            
            # Extract rank (look for "rank:" or similar patterns)
            elif 'rank' in line_lower and ':' in line:
                rank_match = re.search(r'rank\s*:?\s*(.+)', line, re.IGNORECASE)
                if rank_match:
                    credentials['rank'] = rank_match.group(1).strip()
            
            # Extract locker number (look for "locker" or "locker number")
            elif 'locker' in line_lower and ':' in line:
                locker_match = re.search(r'locker\s*(?:number)?\s*:?\s*(.+)', line, re.IGNORECASE)
                if locker_match:
                    credentials['lockerNumber'] = locker_match.group(1).strip()
        
        # Fallback: if we couldn't find structured data, try to extract from unstructured text
        if credentials['name'] == 'Unknown':
            # Look for patterns that might be names (capitalized words)
            name_patterns = re.findall(r'\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*\b', ocr_text)
            if name_patterns:
                # Take the longest match as it's likely to be the full name
                credentials['name'] = max(name_patterns, key=len)
...
def overlay_result_on_image(image_data: bytes, result_text: str) -> bytes:
    """Overlay result text on the OMR sheet image."""
    try:
        # Open image
        image = Image.open(io.BytesIO(image_data))
        ...
        # Calculate text position (bottom center, in the footer area)
        text_bbox = draw.textbbox((0, 0), result_text, font=font)
        text_width = text_bbox[2] - text_bbox[0]
        text_height = text_bbox[3] - text_bbox[1]
        
        # Position text in the footer area (bottom 10% of image, centered)
        ...
        draw.rectangle(background_bbox, fill=(255, 255, 255, 200))
        
        # Draw the result text in red
        draw.text((x, y), result_text, fill=(255, 0, 0), font=font)
        ...
        logger.error(f"Failed to overlay result on image: {str(e)}")
        # Return original image if overlay fails
        return image_data

@router.post("/download-scanned-sheets")
async def download_scanned_sheets(
    images: List[UploadFile] = File(...),
    results_data: str = Form(...),
    db=Depends(get_database)
):
    """Generate PDF with scanned sheets and overlaid results."""
    try:
        import json
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail="Invalid results data format")
        ...
        if len(images) != len(results):
            raise HTTPException(
                status_code=400, 
                detail=f"Number of images ({len(images)}) must match number of results ({len(results)})"
            )
        
        logger.info(f"Processing {len(images)} scanned sheets for PDF generation")
        ...
        for i, (image_file, result) in enumerate(zip(images, results)):
            try:
                # Read image data
                image_data = await image_file.read()
                
                # Extract student credentials using OCR
                credentials = extract_student_credentials_ocr(image_data)
                
                # Update result with extracted credentials
                ...
                modified_image_data = overlay_result_on_image(image_data, result_text)
                
                # Convert to PIL Image for PDF
                pil_image = Image.open(io.BytesIO(modified_image_data))
                ...
                pdf_canvas.drawImage(
                    ImageReader(io.BytesIO(modified_image_data)),
                    x, y, scaled_width, scaled_height
                )
                
                # Add student information as text overlay
                pdf_canvas.setFont("Helvetica-Bold", 12)
                pdf_canvas.setFillColorRGB(0, 0, 0)
                
                info_y = y + scaled_height + 20
                pdf_canvas.drawString(x, info_y, f"Student: {credentials['name']}")
                pdf_canvas.drawString(x, info_y - 15, f"Rank: {credentials['rank']}")
                pdf_canvas.drawString(x, info_y - 30, f"Locker: {credentials['lockerNumber']}")
                
                # Add result summary
                pdf_canvas.setFillColorRGB(1, 0, 0)  # Red color for result
                pdf_canvas.drawString(x, info_y - 50, result_text)
                
                # Start new page if not the last image
                if i < len(images) - 1:
                    pdf_canvas.showPage()
                
                logger.info(f"Processed sheet {i + 1}/{len(images)}")
                
            except Exception as e:
                logger.error(f"Failed to process sheet {i + 1}: {str(e)}")
        ...
        # Generate filename
        exam_name = results[0].get('examName', 'Exam') if results else 'Exam'
        filename = f"{exam_name}_Scanned_Sheets_{datetime.now().strftime('%Y%m%d')}.pdf"
        
        logger.info(f"Generated PDF with {len(images)} scanned sheets")
        ...
        logger.error(f"Failed to generate scanned sheets PDF: {str(e)}")
